...
function gitCommitAndPush {
	...
	replaceTag "v${APPLICATION_VERSION_MAJOR}" "${TEXT}"
	replaceTag "v${APPLICATION_VERSION_MAJOR}"-${APPLICATION_VERSION_EXT} "${TEXT}"
	replaceTag "v${APPLICATION_VERSION_MAJOR}.${APPLICATION_VERSION_MINOR}" "${TEXT}"
	replaceTag "v${APPLICATION_VERSION_MAJOR}.${APPLICATION_VERSION_MINOR}-${APPLICATION_VERSION_EXT}" "${TEXT}"
	replaceTag "v${APPLICATION_VERSION_MAJOR}.${APPLICATION_VERSION_MINOR}.${APPLICATION_VERSION_PATCH}" "${TEXT}"
	replaceTag "v${APPLICATION_VERSION_MAJOR}.${APPLICATION_VERSION_MINOR}.${APPLICATION_VERSION_PATCH}-${APPLICATION_VERSION_EXT}" "${TEXT}"
...
function updateDockerfileBaseImage {
	...
	if [[ ${?} -eq 0 ]]; then
		COMMIT_TEXT="${COMMIT_TEXT}Update base image to ubuntu:${BASE_IMAGE_TAG}.\n"
		BUILD=1
	fi
}
...
updateDockerfileBaseImage jenkins/jenkins "\-lts-jdk11"

APPLICATION_VERSION_MAJOR=$(echo ${DOCKER_IMAGE_TAG} | cut -d "-" -f 1 | cut -d "." -f 1)
APPLICATION_VERSION_MINOR=$(echo ${DOCKER_IMAGE_TAG} | cut -d "-" -f 1 | cut -d "." -f 2)
APPLICATION_VERSION_PATCH=$(echo ${DOCKER_IMAGE_TAG} | cut -d "-" -f 1 | cut -d "." -f 3)
APPLICATION_VERSION_EXT=$(echo ${DOCKER_IMAGE_TAG} | cut -d "-" -f 2-)
